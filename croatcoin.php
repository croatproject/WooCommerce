        ...
        public function email_instructions($order, $sent_to_admin, $plain_text = false)
        {
            ...
            if ($method == "croat") {        
                if (get_post_meta( $order->ID, '_transaction_id', true) == ""){
                    ...
                    $description = '<br><strong>'.__('Pagament en CROAT', 'croatcoin').'</strong><br>';
...
function add_croat_price_to_order_page($order){
            ...
            else 
            {
                ...
                $croats = get_croat_price($preu, $paritat_source, $decimals);
                $croats_qr = get_croat_price_qr($preu, $paritat_source, $decimals);                     
            }           
            ...
            {
            $croat_note .= '<div style="font-size: 13px"><b>Codi QR per al pagament:</b><br><img src="'.$dir_croat.'/qr_pagament.php?wallet='.$wallet.'&preu='.$croats_qr.'"><br>'.__('Pots escanejar directament aquest codi QR amb l\'aplicatiu CROATCore per realizar el pagament ara mateix.', 'croatcoin').'</div>';
            }
...
// Function to get CROAT Parity to € from online source and display price. 
...
// Function to get CROAT Parity to € from online source and display price wihout format for QR Code 

function get_croat_price_qr($preu, $paritat_source, $decimals)
{
    if ($paritat_source == "worldcoinindex")
    {
        $api= "https://www.worldcoinindex.com/apiservice/ticker?key=yCLLGOW7SGGVwi7EPRA3sMe8ewu7BN&label=croatbtc&fiat=eur";
